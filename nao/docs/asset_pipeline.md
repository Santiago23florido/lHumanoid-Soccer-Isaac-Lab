...
1. **Source and license.** NAO H25 V5.0 URDF from `ros-naoqi/nao_robot`, BSD
   3-Clause, vendored verbatim at `nao/assets/urdf/nao.urdf`. Geometry
   from `ros-naoqi/nao_meshes`, CC BY-NC-ND 4.0, fetched locally and never
   committed. See `nao/assets/licenses/README.md`.
...
4. **Convert to USD.** `nao_usd.convert_nao_urdf_to_usd` drives the Isaac Lab
   `UrdfConverter` (`isaacsim.asset.importer.urdf`) into `nao/assets/generated/nao/`.
...
6. **Expose a reusable config.** `NAO_CFG` in `humanoid_transfer/assets/nao.py`.
